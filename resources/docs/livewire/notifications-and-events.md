...
We could send the notification directly from the `store` action on our `chirps.create` component, but that adds more work for the action, which in turn can slow down the request, especially as we'll be querying the database and sending emails.
